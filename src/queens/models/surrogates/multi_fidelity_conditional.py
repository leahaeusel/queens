...
class MultiFidelityConditional(Surrogate):
    """Multi-fidelity conditional for an ensemble of surrogate models.

    Groups the outputs of several simulation models with identical model inputs to one
    multi-fidelity data point in the multi-fidelity space.

    Attributes:
        num_processors_multi_processing (int): Number of processors that should be used in the
                                               multi-processing pool.
        instantiate_probabilistic_mappings (callable): Configured method to instantiate the
        ...
        probabilistic_mapping_obj_lst (lst): List of probabilistic mapping objects, which
                                             ...
                                             individually.
        update_probabilistic_mappings (callable): Configured method to update the probabilistic
            mappings.
    ...
    def __init__(
        self,
        probabilistic_mapping: Surrogate,
        feature_strategy: MultiFidelityFeatureStrategy,
        num_processors_multi_processing=1,
        probabilistic_mapping_type="single",
        training_iterator: "MultiFidelityTrainingData" = None,
        testing_iterator: "MultiFidelityTrainingData" = None,
        ...
        Args:
            ...
            probabilistic_mapping_type (str): Configured method to instantiate the  probabilistic
                                              mapping objects
            feature_strategy (MultiFidelityFeatureStrategy): Object implementing the strategy for
                the selection of informative features from the input space.
        """
        super().__init__(training_iterator, testing_iterator)
        # instantiate probabilistic mapping objects

        valid_probabilistic_mappings_configurations = {
            "single": (
                self.instantiate_single,
                self.predict_single,
                self.update_single_mapping,
            ),
            "per_coordinate": (
                self.instantiate_per_coordinate,
                self.predict_per_coordinate,
                self.update_mappings_per_coordinate,
            ),
            "per_time_step": (
                self.instantiate_per_time_step,
                self.predict_per_time_step,
                self.update_mappings_per_time_step,
            ),
        }
        (
            self.instantiate_probabilistic_mappings,
            self.predict_per_probabilistic_mapping,
            self.update_probabilistic_mappings,
        ) = get_option(valid_probabilistic_mappings_configurations, probabilistic_mapping_type)

        if experimental_data_reader is not None:
            if time_vec is not None or coords_mat is not None:
                _logger.info(
                    "You provided an experimental data reader and time_vec and/or coords_mat. "
                    "In the following, the provided experimental data reader is used and the provided time_vec and coords_mat are ignored."
                )
            _, coords_mat, time_vec = experimental_data_reader.get_experimental_data()
        ...
        if probabilistic_mapping_type == "single" and probabilistic_mapping.is_trained:
            _logger.info(
                "The provided probabilistic mapping is already trained. The MF conditional will "
                "use the provided probabilistic mapping without training it again."
            )
            self.is_trained = True
            self.probabilistic_mapping_obj_lst = [probabilistic_mapping]
        else:
            # Train the probabilistic mappings and build the surrogate model
            self.build_approximation()

    def setup(self, z_train, y_hf_train):
        """Setup the surrogate model.

        Args:
            z_train (np.array): Training inputs for probabilistic mapping.
    ...
    def train(self):
        """Train the ensemble of probabilistic regression models.

        Train the probabilistic mapping objects based on the
        training inputs :math:`\mathcal{D}_f={Y_{HF},Z_{LF}}` per coordinate
        point/measurement point in the inverse problem.

        """
        # check input dimensions
        if self.z_train.ndim != 3:
            raise IndexError("z_lf_train must be a 3d tensor!")

        # if self.z_train.T.shape[0] != self.y_hf_train.T.shape[0]:
        #     raise IndexError("Dimension of z_lf_train and y_hf_train do not agree!")

        # Instantiate list of probabilistic mappings & reshape training data
        (
            self.z_train,
            self.y_hf_train,
            self.probabilistic_mapping_obj_lst,
            ...
            self.z_train,
            self.y_hf_train,
            self.probabilistic_mapping,
        )

        if self.num_processors_multi_processing > 1:
            # Conduct training of probabilistic mappings in parallel
            num_coords = self.z_train.T.shape[2]
            optimized_mapping_states_lst = (
                MultiFidelityConditional.train_probabilistic_mappings_in_parallel(
                    num_coords,
                    self.num_processors_multi_processing,
                    self.probabilistic_mapping_obj_lst,
                )
            )
    ...
    def _get_data_set(self, training_iterator: MultiFidelityTrainingData):
        """Get the training data set from the training iterator.

        Args:
            training_iterator: Iterator to evaluate the low-fidelity and high-fidelity model with
                the purpose of getting training data.

        Returns:
            z_train (np.array): Training inputs for probabilistic mapping. Is composed of the
                low-fidelity model outputs and informative input features.
            y_hf_train (np.array): Training outputs for probabilistic mapping. Corresponds to the
                high-fidelity model outputs.
        """
        self.x_train = training_iterator.X_train
        self.y_lf_train = training_iterator.Y_LF_train
        self.y_hf_train = training_iterator.Y_HF_train

        if self.coords_mat is not None:
            num_coords = self.coords_mat.shape[0]
            self.y_lf_train = self.y_lf_train.reshape(-1, num_coords)
            self.y_hf_train = self.y_hf_train.reshape(-1, num_coords)

        # ----- Set the feature strategy of the probabilistic mapping (select gammas)
        self.z_train = self.feature_strategy.set_feature_strategy(self.y_lf_train, self.x_train)
    ...
    def train_probabilistic_mappings_in_parallel(
        ...
        with mp.get_context("spawn").Pool(processes=num_processors_for_job) as pool:
            # Actual parallel training of the models
            optimized_mapping_states_lst = list(
                tqdm.tqdm(
                    pool.imap(
                        MultiFidelityConditional.optimize_hyper_params,
    ...
    def instantiate_single(self, z_lf_train, y_hf_train, approx):
        """Instantiate a single probabilistic mapping.
        ...
        Returns:
            ...
            probabilistic_mapping_obj_lst (list): List of probabilistic mapping objects
        """
        z_lf_train = np.atleast_2d(z_lf_train)
        if not z_lf_train.shape[0] == 1:
            raise IndexError(
                f"For the 'single' probabilistic mapping configuration, the input z_lf_train must "
                f"have only one row but has {z_lf_train.shape[0]} rows! Abort..."
            )
        approx.setup(z_lf_train[0], np.atleast_2d(y_hf_train))
        probabilistic_mapping_obj_lst = [approx]
        return z_lf_train, y_hf_train, probabilistic_mapping_obj_lst

    def instantiate_per_coordinate(self, z_lf_train, y_hf_train, approx):
    ...
    def instantiate_per_time_step(self, z_lf_train, y_hf_train, approx):
        ...
        _, t_size = MultiFidelityConditional.check_coordinates_return_dimensions(
            z_lf_train, self.time_vec, self.coords_mat
        )

        # prepare LF and HF training array
        z_lf_array = MultiFidelityConditional.prepare_z_lf_for_time_steps(
            z_lf_train, t_size, self.coords_mat
        )
    ...
    def predict_single(self, z_lf, support="y", gradient_bool=False):
        """Predict the response of a single probabilistic mapping.

        Args:
            z_lf (np.array): Low-fidelity feature vector that contains the points on which the
                probabilistic mapping should be evaluated.
                Dimensions: num_samples x num_coordinates
        ...
        if not z_lf.shape[0] == 1:
            raise IndexError(
                f"For the 'single' probabilistic mapping configuration, the input z_lf must "
                f"have only one row but has {z_lf.shape[0]} rows! Abort..."
            )
        output = self.probabilistic_mapping_obj_lst[0].predict(
            z_lf[0],
            support=support,
    ...
    def predict_per_coordinate(self, z_lf, support="y", gradient_bool=False):
        r"""Map the LF features to the response of the probabilistic mappings.

        Here a probabilistic mapping per coordinate combination (time and space)
        is evaluated and the mean and variance are predicted. The gradients of the mean and the variance of the underlying probabilistic mapping are also evaluated if `gradient_bool` is True.

        Args:
            z_lf (np.array): Low-fidelity feature vector that contains the points on which the
                probabilistic mapping should be evaluated.
                Dimensions: (gamma_dim x) num_samples x coord_dim:

                    - Rows: Different multi-fidelity vector/points (each row is one multi-fidelity
                        point).
        ...
        Returns:
            output (dict): Dictionary with the following entries:
                ...
                - grad_variance (np.array): Gradient matrix for the mean prediction.
                                     ...
                                     vector entries per column
        """
        if len(self.probabilistic_mapping_obj_lst) != z_lf.T.shape[0]:
            raise IndexError(
        ...
        for z_test_per_coordinate, probabilistic_mapping_obj in zip(
            z_lf.T, self.probabilistic_mapping_obj_lst, strict=True
        ):
            if z_test_per_coordinate.ndim == 1:
                z_test_per_coordinate = np.atleast_2d(z_test_per_coordinate).T

            output_per_coordinate = probabilistic_mapping_obj.predict(
                z_test_per_coordinate,
                support=support,
                gradient_bool=gradient_bool,
            )

            mean_y_hf_given_z_lf.append(output_per_coordinate["result"].squeeze())
            var_y_hf_given_z_lf.append(output_per_coordinate["variance"].squeeze())

            if gradient_bool:
                grad_mean.append(output_per_coordinate["grad_mean"].squeeze().T)
                grad_variance.append(output_per_coordinate["grad_var"].squeeze().T)
        ...
        if gradient_bool:
            output["grad_mean"] = grad_mean
            output["grad_var"] = grad_variance

        return output

    def predict_per_time_step(self, z_lf, support="y", gradient_bool=False):
        r"""Map the LF features to a probabilistic response for the hf model.

        Here a probabilistic mapping per time-step but for all locations combined is evaluated and
        the mean and variance are predicted. The gradients of the mean and the variance of the
        underlying probabilistic mapping are also evaluated if `gradient_bool` is True.
        ...
        # determine the number of time steps and check coordinate compliance
        num_coords, t_size = MultiFidelityConditional.check_coordinates_return_dimensions(
            z_lf, self.time_vec, self.coords_mat
        )
        z_lf_array = MultiFidelityConditional.prepare_z_lf_for_time_steps(
            z_lf, t_size, self.coords_mat
        )
        ...
        (
            ...
            grad_mean,
            grad_variance,
        ) = MultiFidelityConditional.iterate_over_time_steps(
            z_lf_array,
            support,
            num_coords,
            self.probabilistic_mapping_obj_lst,
        ...
        if gradient_bool:
            output["grad_mean"] = grad_mean.swapaxes(1, 2)
            output["grad_var"] = grad_variance.swapaxes(1, 2)

        return output

    def update_single_mapping(self, z_lf_train, y_hf_train):
        """Update a single probabilistic mapping.
        ...
        self.probabilistic_mapping_obj_lst[0].update_training_data(
            z_lf_train[0], np.atleast_2d(y_hf_train)
        )
        return z_lf_train, y_hf_train, self.probabilistic_mapping_obj_lst

    def update_mappings_per_coordinate(self, probabilistic_mapping_obj_lst, z_lf_train, y_hf_train):
    ...
    def update_mappings_per_time_step(self, probabilistic_mapping_obj_lst, z_lf_array, y_hf_array):
        ...
        # determine the number of time steps and check coordinate compliance
        _, t_size = MultiFidelityConditional.check_coordinates_return_dimensions(
            z_lf_array, self.time_vec, self.coords_mat
        )

        # prepare LF and HF training array
        z_lf_array = MultiFidelityConditional.prepare_z_lf_for_time_steps(
            z_lf_array, t_size, self.coords_mat
        )
    ...
    @staticmethod
    def optimize_hyper_params(probabilistic_mapping: Surrogate):
    ...
    def predict(self, z_lf, support="y", gradient_bool=False):
        """Predict."""
        if not self.is_trained:
            raise RuntimeError(
                "The surrogate model has not been trained yet! Please call the train method before calling predict. Abort..."
            )

        return self.predict_per_probabilistic_mapping(z_lf, support, gradient_bool)
