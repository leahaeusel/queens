...
class FullRankNormal(Variational):
    ...
    def construct_variational_parameters(  # pylint: disable=arguments-differ
        self, mean: ArrayNxN, covariance: ArrayNxN
    ) -> np.ndarray:
        """Construct the variational parameters from mean and covariance.

        Args:
            mean: Mean values of the distribution (n_dim x 1)
            covariance: Covariance matrix of the distribution (n_dim x n_dim)
    ...
    ) -> tuple[ArrayNxN, ArrayNxN, ArrayNxN]:
        """Reconstruct mean value, covariance and its Cholesky decomposition.

        Args:
            variational_parameters: Variational parameters
            return_cholesky: Return the L if desired
        Returns:
            Mean value of the distribution (n_dim x 1)
            Covariance of the distribution (n_dim x n_dim)
            Cholesky decomposition of the covariance matrix (n_dim x n_dim)
        """
    ...
    def _grad_reconstruct_distribution_parameters(self) -> np.ndarray:
    ...
    def draw(self, variational_parameters: np.ndarray, n_draws: int = 1) -> np.ndarray:
        ...
        """
        mean, _, cholesky = self.reconstruct_distribution_parameters(variational_parameters)
    ...
    def logpdf(self, variational_parameters: np.ndarray, x: np.ndarray) -> np.ndarray:
        """Log-PDF evaluated using the at samples *x*.
        ...
        """
        mean, cov, cholesky = self.reconstruct_distribution_parameters(variational_parameters)
        x = np.atleast_2d(x)
    ...
    def grad_params_logpdf(self, variational_parameters: np.ndarray, x: np.ndarray) -> np.ndarray:
    ...
    def total_grad_params_logpdf(
        self, variational_parameters: np.ndarray, standard_normal_sample_batch: np.ndarray
    ) -> np.ndarray:
    ...
    def grad_sample_logpdf(
        self, variational_parameters: np.ndarray, sample_batch: np.ndarray
    ) -> np.ndarray:
    ...
    def fisher_information_matrix(self, variational_parameters: np.ndarray) -> np.ndarray:
        ...
        """
        _, cov, cholesky = self.reconstruct_distribution_parameters(variational_parameters)
