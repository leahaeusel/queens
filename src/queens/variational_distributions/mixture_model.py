...
class MixtureModel(Variational):
    ...
    def construct_variational_parameters(  # pylint: disable=arguments-differ
        self, component_parameters: list, weights: np.ndarray
    ) -> np.ndarray:
        ...
        for component_parameter in component_parameters:
            variational_parameters.append(
                self.base_distribution.construct_variational_parameters(*component_parameter)
            )
    ...
    def fisher_information_matrix(
        self, variational_parameters: np.ndarray, n_samples: int = 10000
    ) -> np.ndarray:
