...
class Particle(Variational):
    ...
    def construct_variational_parameters(  # pylint: disable=arguments-differ
        self, probabilities: np.ndarray, sample_space: np.ndarray
    ) -> np.ndarray:
    ...
    def draw(self, variational_parameters: np.ndarray, n_draws: int = 1) -> ArrayNxN:
        ...
        Returns:
            Samples (n_draws x n_dim)
        """
        self.reconstruct_distribution_parameters(variational_parameters)
        return self.particles_obj.draw(n_draws)

    def logpdf(self, variational_parameters: np.ndarray, x: ArrayNxN) -> np.ndarray:
        """Evaluate the natural logarithm of the PDF.

        Args:
            variational_parameters: Variational parameters of the distribution
            x: Locations at which to evaluate the distribution (n_samples x n_dim)
    ...
    def pdf(self, variational_parameters: np.ndarray, x: ArrayNxN) -> np.ndarray:
    ...
    def grad_params_logpdf(self, variational_parameters: np.ndarray, x: ArrayNxN) -> np.ndarray:
    ...
    def fisher_information_matrix(self, variational_parameters: np.ndarray) -> ArrayNxN:
