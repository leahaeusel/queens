...
#
"""Unit tests for MultiFidelityMapping iterator."""
...
# ------------ fixtures and params -----------------------------------
@pytest.fixture(name="default_multi_fidelity_mapping")
def fixture_default_multi_fidelity_mapping(
    dummy_simulation_model, get_patched_multi_fidelity_mapping, default_parameters_uniform_2d
):
    """Dummy iterator for testing."""
    hf_model = dummy_simulation_model
    lf_model = dummy_simulation_model

    iterator = get_patched_multi_fidelity_mapping(default_parameters_uniform_2d, hf_model, lf_model)
...
def test_calculate_optimal_x_train(mocker, default_parameters_uniform_2d):
    ...
    **Note:** Here we return the input arguments of the design method to
    ...
    mo_1 = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping.get_design_method",
        return_value=my_mock_design,
    )

    x_train, (arg0, arg1) = MultiFidelityTrainingData.calculate_initial_x_train(
...
def test_get_design_method(mocker):
    ...
    mo_1 = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping.random_design",
        return_value=my_mock_design,
    )

    design = MultiFidelityTrainingData.get_design_method(initial_design_dict)
    assert design is mo_1

    # test invalid design
    with pytest.raises(NotImplementedError):
        initial_design_dict = {"type": "randommm"}
        MultiFidelityTrainingData.get_design_method(initial_design_dict)
...
def test_random_design(default_parameters_uniform_2d):
    ...
    x_out = MultiFidelityTrainingData.random_design(
        initial_design_dict, default_parameters_uniform_2d
    )
...
def test_core_run(default_multi_fidelity_mapping, mocker):
    """Test the core run of the iterator."""
    z_train_in = np.array([[4], [5]])
    y_hf_train_in = np.array([[2.2], [3.3]])

    mo_1 = mocker.patch("queens.iterators.multi_fidelity_mapping.MultiFidelityMapping.eval_model")
    mo_2 = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping.set_feature_strategy",
        return_value=z_train_in,
    )

    z_train_out, y_hf_train_out = default_multi_fidelity_mapping.core_run()

    # Actual tests / asserts
    mo_1.assert_called_once()
    mo_2.assert_called_once()
    np.testing.assert_array_equal(mo_2.call_args[0][0], default_multi_fidelity_mapping.Y_LF_train)
    np.testing.assert_array_equal(mo_2.call_args[0][1], default_multi_fidelity_mapping.X_train)
    np.testing.assert_array_equal(
        mo_2.call_args[0][2], default_multi_fidelity_mapping.coords_experimental_data
    )
    np.testing.assert_array_equal(z_train_out, z_train_in)
    np.testing.assert_array_equal(y_hf_train_out, y_hf_train_in)


def test_evaluate_LF_model_for_X_train(default_multi_fidelity_mapping):
    """Test evaluation of LF model with test data."""
    with patch.object(
        default_multi_fidelity_mapping.lf_model,
        "evaluate",
        return_value={"result": np.array([1, 1])},
    ) as mo_1:
        default_multi_fidelity_mapping.evaluate_LF_model_for_X_train()

        mo_1.assert_called_once()
        np.testing.assert_array_equal(np.array([[1, 1]]), default_multi_fidelity_mapping.Y_LF_train)


def test_evaluate_HF_model_for_X_train(default_multi_fidelity_mapping):
    """Test evaluation of HF model with test data."""
    with patch.object(
        default_multi_fidelity_mapping.hf_model,
        "evaluate",
        return_value={"result": np.array([1, 1])},
    ) as mo_2:
        default_multi_fidelity_mapping.evaluate_HF_model_for_X_train()

        # Actual asserts / tests
        mo_2.assert_called_once()
        np.testing.assert_array_equal(np.array([[1, 1]]), default_multi_fidelity_mapping.Y_HF_train)


def test_set_feature_strategy(default_multi_fidelity_mapping, mocker):
    ...
    with pytest.raises(AssertionError):
        default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
    ...
    x_mat = np.array([[4, 5, 6]])
    coords_mat = np.array([[7, 8, 9]])
    default_multi_fidelity_mapping.features_config = "dummy"
    ...
    coords_mat = np.array([[7, 8, 9]])
    mo_man = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping._get_man_features",
        return_value=(1, 1),
    )
    mo_coord = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping._get_coord_features",
        return_value=(1, 1),
    )
    mo_no = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping._get_no_features",
        return_value=(1, 1),
    )
    mo_time = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping._get_time_features",
        return_value=(1, 1),
    )

    default_multi_fidelity_mapping.features_config = "man_features"
    default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)

    default_multi_fidelity_mapping.features_config = "opt_features"
    default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)

    default_multi_fidelity_mapping.features_config = "coord_features"
    default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)

    default_multi_fidelity_mapping.features_config = "no_features"
    default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)

    default_multi_fidelity_mapping.features_config = "time_features"
    default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
...
def test_get_man_features(default_multi_fidelity_mapping):
    ...
    default_multi_fidelity_mapping.features_config = "man_features"
    with pytest.raises(AssertionError):
        z_mat = default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
    ...
    coords_mat = np.array([[7, 8, 9], [10, 11, 12], [13, 14, 15]])
    default_multi_fidelity_mapping.features_config = "man_features"
    default_multi_fidelity_mapping.x_cols = 0
    with pytest.raises(AssertionError):
        z_mat = default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
    ...
    coords_mat = np.array([[7, 8, 9], [10, 11, 12], [13, 14, 15]])
    default_multi_fidelity_mapping.features_config = "man_features"
    default_multi_fidelity_mapping.x_cols = []
    with pytest.raises(AssertionError):
        z_mat = default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
...
def test_get_coord_features(default_multi_fidelity_mapping):
    ...
    default_multi_fidelity_mapping.features_config = "coord_features"
    default_multi_fidelity_mapping.coord_cols = []
    with pytest.raises(AssertionError):
        z_mat = default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)

    # test coord_features without list format for cols
    default_multi_fidelity_mapping.features_config = "coord_features"
    default_multi_fidelity_mapping.coord_cols = 0
    with pytest.raises(AssertionError):
        z_mat = default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
...
def test_get_no_features(default_multi_fidelity_mapping):
...
def test_get_time_features(default_multi_fidelity_mapping):
    ...
    default_multi_fidelity_mapping.features_config = "time_features"
    default_multi_fidelity_mapping.time_vec = np.linspace(0, 10, y_lf_mat.shape[1])
    z_mat = default_multi_fidelity_mapping.set_feature_strategy(y_lf_mat, x_mat, coords_mat)
    np.testing.assert_array_almost_equal(z_mat, expected_z_mat, decimal=4)


def test_update_probabilistic_mapping_with_features(default_multi_fidelity_mapping):
    """Test for updating with optimal informative features."""
    with pytest.raises(NotImplementedError):
        default_multi_fidelity_mapping.update_probabilistic_mapping_with_features()


def test_eval_model(default_multi_fidelity_mapping, mocker):
    """Test for evaluating the underlying model."""
    mo_1 = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping.evaluate_LF_model_for_X_train"
    )
    mo_2 = mocker.patch(
        "queens.iterators.multi_fidelity_mapping.MultiFidelityMapping.evaluate_HF_model_for_X_train"
    )
    default_multi_fidelity_mapping.eval_model()
